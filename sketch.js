...
function preload() {
  ...
  sampleSounds = [
    {
      name: "Sample Song 1",
      sound: loadSound("assets/To_Build_A_Home_Radio_Version_KLICKAUD.mp3")
    },
  ];
}
...
function setup() {
  ...
  for (let i = 0; i < sampleSounds.length; i++) {
    const fft = new p5.FFT(0.9, 64);
    const amp = new p5.Amplitude(0.9);

    sampleSounds[i].sound.connect(fft);
    sampleSounds[i].sound.connect(amp);

    songs.push({
      name: sampleSounds[i].name,
      sound: sampleSounds[i].sound,
      fft: fft,
      amp: amp
    });
  }
